...
const { webhook: shiprocketWebhook } = require("./controllers/shipmentController");
...
const INLINE_SAFE = new Set([".jpg", ".jpeg", ".png", ".webp", ".gif", ".avif", ".pdf", ".mp4", ".webm"]);
...
app.get(`${API}/health`, (req, res) => {
  res.json({ success: true, service: "woven-essence-backend", time: new Date().toISOString() });
});
...
if (isProd || hasBuild) {
  ...
  console.log("[serve] frontend build -> /  (shop)  and  /admin  (admin panel)");
} else {
